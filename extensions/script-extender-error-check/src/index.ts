...
async function checkForErrors(api: types.IExtensionApi) {
    ...
    logPaths.map(async (filePath) => {
      ...
      try {
        ...
        if (Math.round(logTime / 1000) > launchTime) {
          const logFile = await fs.readFileAsync(filePath, {
            encoding: "utf8",
          });
        ...
        log(
          err.code === "ENOENT" ? "info" : "error",
          "Failed to check for script extender errors",
          err.message,
        );
      }
    }),
  ...
  if (errorInstances.length > 0) {
    ...
    const renderError = (input) => {
      const modName =
        input.modId !== undefined && mods[input.modId] !== undefined
    ...
    const buildLogErrorString = (errLog: IErrorLog): string => {
      ...
      return api.translate(
        "[url={{logPathURI}}]{{logPath}}[/url] {{pathSep}} [url={{logURI}}]{{logName}}[/url]<br/>" +
      ...
      actions: [
        {
          ...
          action: () =>
            api.showDialog(
              "info",
              "Script extender plugin errors",
              {
                bbcode:
                  api.translate(
                    "Last time you ran the game or Creation kit, one or more script extender " +
  ...
  lines.forEach((line) => {
    ...
    if (message !== undefined) {
      ...
      if (match !== null) {
        ...
        errorArray.push({
          dllName,
          message: messageFromCode(parseInt(match[2], 10)),
        });
      }
    }
  });
