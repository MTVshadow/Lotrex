...
function requiresLauncher(gamePath, store) {
  ...
  return util.GameStoreHelper.findByAppId([MS_ID], "xbox")
    .then((gameEntry) => {
      util.getNormalizeFunc(gamePath).then((norm) => (normalize = norm));
      return gameEntry;
    })
    .then((gameEntry) => {
...
function main(context) {
  context.registerGame({
    ...
    queryArgs: {
      ...
      registry: [
        {
          id: "HKEY_LOCAL_MACHINE:Software\\Wow6432Node\\Bethesda Softworks\\Skyrim Special Edition:Installed Path",
        },
      ],
    },
