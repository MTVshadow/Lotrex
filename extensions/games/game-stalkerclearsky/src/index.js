...
// Nexus Mods id для S.T.A.L.K.E.R.: Clear Sky (https://www.nexusmods.com/stalkerclearsky)
const GAME_ID = "stalkerclearsky";
const STEAM_APP_ID = "20510";
const GOG_APP_ID = "1435826982";
        ...
        .catch(() =>
          util.GameStoreHelper.findByRegistry(
            [
              {
                hive: "HKLM",
                key: "Software\\GSC Game World\\STALKER-CS",
                value: "InstallPath",
              },
              {
                hive: "HKLM",
                key: "Software\\GOG.com\\Games\\1435826982",
                value: "PATH",
              },
            ],
            GAME_EXE,
...
async function prepareForModding(discovery) {
  ...
  try {
    ...
    if (changed) {
      ...
      log("info", "Automatically enabled mod loading in fsgame.ltx for Clear Sky", { ltxPath });
    }
  } catch (err) {
    log("warn", "Could not check/update fsgame.ltx for Clear Sky", { error: err.message });
  }
}
...
function main(context) {
  ...
  context.registerInstaller("stalkerclearsky-mod", 25, testSupportedContent, installContent);
