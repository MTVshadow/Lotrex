...
| Steam validation race handling          |     ✅      |     ✅     |         —         |     —     |    —    | Every manifest-driven unlink rechecks target identity immediately before removal. Missing targets remain harmless, while a Steam/user-replaced target aborts with `EDEPLOYMENTTARGETCHANGED`, remains on disk, and stays represented by the previous manifest entry. A synthetic lifecycle regression test is written but has not completed under the hanging renderer runner.                                     |
...
| Long path and filename corpus           |      —      |     —      |         —         |     —     |    —    | Install/deploy/purge handle filesystem and Proton path limits with actionable rejection before partial writes.                                                                                                                                                                                                                                                                                      |
| Unicode NFC/NFD and locale casefolding  |   Partial   |  Partial   |         —         |  Partial  |    —    | Collision corpus covers NFC/NFD, Turkish case mapping, non-Latin names, and mixed Windows/POSIX separators.                                                                                                                                                                                                                                                                                         |
| Broken and hostile symlink handling     |   Partial   |  Partial   |         —         |  Partial  |    —    | Broken links, link loops, destination replacement, and symlink races cannot escape allowed roots or overwrite unrelated files.                                                                                                                                                                                                                                                                      |
