...
|     5 | Critical Linux workflows          | Partial | All 167 source keys in `health_check`, 37 keys for the Linux Setup assistant, and 47 keys for deployment recovery in `mod_management` are translated, covering dependencies, Steam/Proton discovery, prefixes, filesystems, disk space, Flatpak access, remediation, runtime configuration, staging paths, progress indicators, journal inspection, and rollback/resume confirmations. Navigation and installer surfaces remain. |
...
|     9 | Automation and regression gates   |   ✅    | The enhanced `i18n:coverage` command reports every bundled namespace and validates 542/542 keys (100.0% coverage) with zero token/syntax errors, enforces strict mode, and catches obsolete keys and untranslated copies. Comprehensive vitest regression suites cover restart-free switching and table localization.                                                                                                            |
