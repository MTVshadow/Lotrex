...
| ----: | ---------------------------------- | :------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Unified game identity              | Complete | Implemented `src/renderer/src/util/linux/gameIdentity/` with canonical invariant identity, distinct edition separation, deduplication across Steam/Heroic/Lutris, and relocation reconciliation preserving Lotrex profile bindings. Automated in `gameIdentity.test.ts` (5/5 passing).                                                                                                                                                                                                                                                                  |
|     2 | Capability-based game adapter API  | Complete | Implemented `src/renderer/src/util/linux/gameAdapters/` with 10 versioned capabilities, explicit unsupported capability visibility, dynamic registry lifecycle (install, upgrade, disable, remove), and profile durability. Automated in `gameAdapter.test.ts` (7/7 passing).                                                                                                                                                                                                                                                                           |
|     3 | Launcher/provider boundary         | Complete | Implemented `src/renderer/src/util/linux/launcherProviders/` with Steam, Heroic, Lutris, Bottles, and Manual provider adapters. Normalizes executable, Proton/Wine prefix, environment, and structured arguments without shell concatenation. Automated in `launcherProviders.test.ts` (8/8 passing).                                                                                                                                                                                                                                                   |
|     4 | Unified library UI                 | Complete | Implemented `src/renderer/src/util/linux/unifiedLibrary/` and `src/renderer/src/views/UnifiedLibraryView.tsx` with multi-launcher/multi-disk/distinct-edition deduplication, explainable launch & mod readiness diagnostics, multi-axis filtering, and non-destructive manual correction overlays preserving launcher manifests. Automated in `unifiedLibrary.test.ts` and `UnifiedLibraryView.test.tsx` (21/21 passing).                                                                                                                               |
|     5 | Modular mod pipeline               | Complete | Implemented `src/renderer/src/util/linux/modPipeline/` with declarative archive inspection, SHA-256 staging, priority conflict resolution, transactional atomic deployment & rollback, declarative load order (plugins.txt & mods.json), generated-file tracking, and vanilla-restoring purge. Demonstrated serving 2 structurally different reference games (Skyrim SE Proton & Native Linux Engine). Automated in `modPipeline.test.ts` (7/7 passing).                                                                                                |
|     6 | Runtime and tool orchestration     | Complete | Implemented `src/renderer/src/util/linux/toolOrchestration/` with single trusted launch-plan contract for SKSE, LOOT, xEdit, BodySlide, Nemesis/Pandora, per-tool runtime/prefix overrides, inherited environment, vector-based argument arrays without shell concatenation, process group cancellation, timeout supervisor, and NTSTATUS/signal diagnostics. Automated in `toolOrchestration.test.ts` (9/9 passing).                                                                                                                                   |
|     7 | User automation and custom scripts | Complete | Implemented `src/renderer/src/util/linux/customScripts/` with 13 mandatory security controls, ephemeral 0o700 workspaces, pre-execution preview, transactional rollback, save backups, and 35/35 automated tests covering benign fixtures and adversarial vectors.                                                                                                                                                                                                                                                                                      |
|     8 | Extension SDK and validation       | Complete | Implemented `src/renderer/src/util/linux/adapterSdk/` with typed manifests, semver schema compatibility validation, 10-capability automated conformance suite, permission grants, isolated localization namespaces, diagnostics, sample native & Proton reference adapters, and adapter scaffolder. Enables zero-core-modification game onboarding. Automated in `adapterSdk.test.ts` (7/7 passing).                                                                                                                                                    |
|     9 | Game support tiers and catalog     | Complete | Implemented `src/renderer/src/util/linux/supportCatalog/` tracking `unsupported`, `experimental`, `community-tested`, and `supported` per (gameId, editionId, storeId, platform) tuple. Records maintainer, adapter version, tested artifact, distro/runtime/deployment matrix, known limitations, and verification history. Implemented automatic staleness downgrading (>90d, >180d) and 10-stage reproducible promotion verification. Automated in `supportCatalog.test.ts` (10/10 passing).                                                         |
|    10 | Reference-game pilot               | Complete | Implemented `src/renderer/src/util/linux/referencePilot/` executing complete 12-stage lifecycle validation on 1 Native Linux game and 1 Windows/Proton game (Skyrim SE): discovery, profile creation, mod install & SHA-256 staging, conflict resolution, transactional deployment, load order sync (mods.json & plugins.txt), tool/script orchestration (SKSE64, patcher), launch planning, in-game asset verification, vanilla purge restoration, atomic recovery rollback, and restart/upgrade. Automated in `referencePilot.test.ts` (4/4 passing). |
|    11 | Controlled library expansion       | Complete | Implemented `src/renderer/src/util/linux/cohortExpansion/` with small-batch cohort management (enforcing maxBatchSize), 7 mandatory intake admission gates, automated `SmokeScenarioRunner` executing repeatable smoke tests, `GameExtensionRecommender` strictly enforcing Rule 637 (never advertising mod support without active adapter) and suggesting reviewed extensions or SDK scaffolding, and reference cohorts (Pilots & Bethesda Expansion). Automated in `cohortExpansion.test.ts` (13/13 passing).                                         |
...
| ------------------------------------ | :------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| No automatic execution               | Complete | Every new/imported/updated script starts disabled. Only direct user action can approve and execute. Regression tests in `customScriptSecurityGate.test.ts` cover mod, collection, adapter, profile, update, and dependency-install triggers and verify none can execute. |
| Minimum filesystem permissions       | Complete | Distinct declared read and write roots enforced in `forbiddenAreas.ts` and `transactionalWrites.ts`. Undeclared access fails closed and throws `PermissionBoundaryError` with diagnostic details.                                                                        |
| Forbidden host areas                 | Complete | Canonical path and symlink resolution in `forbiddenAreas.ts` strictly rejects `/`, whole `$HOME`, `/etc`, `/usr`, `/bin`, `/boot`, `/dev`, `/proc`, `/sys`, `/run`, `/var`, SSH/GPG directories, keyrings, and browser profiles before mutation.                         |
| Exact pre-execution preview          | Complete | `generateScriptPreview()` exposes script identity, content/manifest hashes, interpreter, argument vector, roots, network access, timeout, expected outputs, transaction guarantees, and approval status before execution.                                                |
| Transactional managed-file writes    | Complete | `prepareTransactionalPlan()` snapshots pre-execution files; `verifyAndCommitWrites()` attributes outputs; `rollbackTransactionalWrites()` atomically restores backups and unlinks newly created files upon failure or cancellation.                                      |
| Isolated Proton/Wine prefixes        | Complete | `assertPrefixIsolation()` binds execution strictly to the configured prefix; rejects Wine `dosdevices/z:` escapes and cross-prefix writes to other game prefixes.                                                                                                        |
| Sanitized process environment        | Complete | `sanitizeScriptEnvironment()` enforces minimal allowlist, strips secrets (`TOKEN`, `SECRET`, `PASSWORD`, `KEY`), removes `DBUS_SESSION_BUS_ADDRESS`, and `redactScriptOutput()` redacts tokens, secrets, and home paths from output streams.                             |
| Timeout and process-tree termination | Complete | Detached process group supervision in `runScriptProcessWithTimeout()` invokes `signalManagedProcessTree(pid, "SIGTERM")` with SIGKILL escalation on timeout or cancellation, rolling back transactional state.                                                           |
| Approval invalidation                | Complete | `validateScriptApproval()` binds approval to content hash (SHA-256) and manifest hash; content or manifest/argument/interpreter modifications immediately revoke approval (`ApprovalRevokedError`).                                                                      |
| Safe mode                            | Complete | `isSafeModeActive()` checks `--safe-mode`, `VORTEX_SAFE_MODE=1`, or context flag, immediately aborting execution with `SafeModeActiveError` while preserving inspection and log export.                                                                                  |
| Save backup before risky actions     | Complete | `createAtomicSaveBackup()` detects game save roots, creates atomic identified backup directory, prunes via retention policy, and `restoreSaveBackup()` restores state on failure without touching cloud tokens.                                                          |
| Dry run and declared effects         | Complete | `dryRun: true` in context returns simulated execution preview and declared effects without touching filesystem; predictable effects labeled.                                                                                                                             |
| No `sudo` password handling          | Complete | `assertNoPrivilegeEscalation()` detects and rejects `sudo`, `pkexec`, `doas`, `su`, `gksu`, `kdesu`, and `runas` in interpreter, arguments, or script content (`PrivilegeEscalationForbiddenError`); stdin is closed.                                                    |
