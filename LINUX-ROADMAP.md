...
|    11 | Controlled library expansion       | Complete | Implemented `src/renderer/src/util/linux/cohortExpansion/` with strict small-batch cohort management (enforcing maxBatchSize), 7 mandatory intake admission gates (SDK conformance >=90%, named maintainer, legal redistributable fixture clearance, well-understood mod formats, repeatable smoke test, and retained lifecycle evidence), catalog graduation, and bloat rejection. Automated in `cohortExpansion.test.ts` (6/6 passing).                                                                                                               |
