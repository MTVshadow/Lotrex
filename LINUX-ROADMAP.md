...
## Immediate priority — Lotrex library navigation isolation

This is the first item to fix before any further Unified Library feature work.

| Priority | Status | Defect                                                                                                                               | Required fix and acceptance evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| :------: | :----: | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  **P0**  |  Open  | After navigating away from **Lotrex library**, its page remains rendered and is visually joined above or below the destination page. | Audit the main-page registration and `Page`/layout ownership, reproduce transitions in both directions, and ensure the Unified Library unmounts or becomes inactive exactly like existing main pages. Add a navigation regression covering repeated `Lotrex library -> another page -> Lotrex library` transitions, then manually verify the packaged Linux UI at common window sizes. No persistence, localization, provider migration, or additional library feature work starts until this defect is closed. |
...
1. Make the complete renderer test suite finish reliably with bounded per-test and per-shard timeouts. (Partial: the latest audit completed 275 files with 274 passed, 1 environment-dependent failure in `environmentAssessment.test.ts`, 2706 tests passed, and 9 skipped. The `/tmp` mount was reported read-only by the test environment; reproduce and make the fixture independent of host mount flags before restoring Completed.)
2. Report open handles and the last active test when a shard times out.
3. Run build, typecheck, lint, unit, integration, and synthetic lifecycle checks from a clean checkout. (Current working-tree evidence: renderer typecheck, lint, build, formatter, and 57 focused integration/security/UI tests pass. A clean-checkout full run and the single environment-assessment failure remain.)
...
| ----: | ---------------------------------- | :----------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Unified game identity              |   Partial    | Identity contracts and relocation reconciliation are implemented. The production bridge now prevents same-edition multi-install ID collisions and feeds real Vortex discoveries/profiles into the unified model. Next: persist provider fingerprints and installation IDs so automatic relocation survives restart without path identity.                                                                                                                                           |
|     2 | Capability-based game adapter API  |   Partial    | The versioned registry exists. A read-only legacy adapter bridge now exposes existing Vortex game extensions without replacing their proven behavior. Next: persist adapter lifecycle/state and incrementally map real extension capabilities instead of inferring completion from declarations.                                                                                                                                                                                    |
|     3 | Launcher/provider boundary         |   Partial    | Standalone provider modules and tests exist, but normal launches still use the established `StarterInfo`/unified launch path. The library deliberately delegates to that production path. Next: adapt one provider at a time behind equivalence tests while preserving store-owned URI/DRM behavior.                                                                                                                                                                                |
|     4 | Unified library UI                 |   Partial    | The library is mounted as a Linux main page and consumes real Redux discovery/profile state through `legacyGameBridge.ts`; launch and in-memory correction actions are connected. **First unblocker:** fix the P0 navigation-isolation defect where this page remains visible after switching pages, add a repeated-navigation regression, and verify it in the packaged UI. Only then localize the UI, persist validated corrections, add refresh/error states, and verify launch. |
|     5 | Modular mod pipeline               |  Prototype   | Contracts and synthetic transaction tests exist, but this parallel deployer is not connected and lacks the safety/recovery maturity of production mod management. Next: reuse the existing journaled deployment engine through an adapter boundary; do not introduce a second writer to game directories.                                                                                                                                                                           |
|     6 | Runtime and tool orchestration     |  Prototype   | Launch-plan and supervision helpers exist without production consumers. Next: bridge the existing Tools/`StarterInfo` path, compare generated plans, then migrate only when Native/Proton/Wine behavior and cancellation are equivalent.                                                                                                                                                                                                                                            |
|     7 | User automation and custom scripts | Blocked-safe | Preview, approval, cleanup, and test helpers exist, but manifest path checks are not an OS sandbox and rollback cannot safely contain arbitrary code. Production execution now fails closed until a real filesystem/process/network isolation backend is supplied. Dry-run and inspection may be integrated first.                                                                                                                                                                  |
|     8 | Extension SDK and validation       |  Prototype   | Typed schema, scaffolder, samples, and conformance tests exist. Next: connect validation to the actual extension-loading lifecycle, persist compatibility decisions, isolate failures, and publish a versioned developer contract before claiming zero-core onboarding.                                                                                                                                                                                                             |
|     9 | Game support tiers and catalog     |  Prototype   | Catalog evaluation exists in memory, but no support claim may be produced from self-authored synthetic evidence. Next: persist signed evidence references and derive tiers only from retained packaged runs, issue state, adapter version, and artifact checksum.                                                                                                                                                                                                                   |
|    10 | Reference-game pilot               |  Synthetic   | The current runner exercises in-memory/filesystem fixtures and must not register either fixture as a supported real game. Next: replace self-generated artifact IDs and simulated in-game checks with an external harness consuming one immutable packaged build and retained Native/Proton evidence.                                                                                                                                                                               |
|    11 | Controlled library expansion       |  Synthetic   | Cohort and recommendation policy helpers exist; the smoke runner currently writes its own mock executable and output. Next: execute adapter-owned fixtures in an isolated harness, require independent expected results, and admit cohorts only after the Phase 10 packaged pilot is complete.                                                                                                                                                                                      |
...
| ------------------------------------ | :---------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No automatic execution               |   Partial   | Execution requires a direct-user trigger and approval in the library API, while no product UI currently enables scripts. Production execution additionally fails closed without an OS sandbox. Next: integrate proposal/approval storage and prove every import/update path remains disabled. |
| Minimum filesystem permissions       |   Blocked   | Manifest paths are validated but not enforced against the spawned process. Next: implement an OS isolation backend (for example a capability-probed bubblewrap/portal boundary) and adversarially prove undeclared reads and writes fail.                                                     |
| Forbidden host areas                 |   Blocked   | Declaration validation rejects known sensitive paths, but arbitrary code still has the user's ambient filesystem access without a sandbox. Keep production execution disabled until host access is enforced rather than inferred.                                                             |
| Exact pre-execution preview          | Implemented | `generateScriptPreview()` produces the required model. Next: mount a localized approval UI, show diffs after changes, persist the exact approved record, and verify keyboard/screen-reader behavior.                                                                                          |
| Transactional managed-file writes    |   Blocked   | Current snapshots back up declared outputs, not every pre-existing file an unrestricted process could modify or delete; rollback is not atomic and may remove concurrently created files. Replace post-hoc scanning with enforced write isolation and the production deployment journal.      |
| Isolated Proton/Wine prefixes        |   Blocked   | Path declarations are checked, but the child can still access other prefixes and Wine host mappings. Require OS-level mount isolation and test real Wine/Proton processes before enabling execution.                                                                                          |
| Sanitized process environment        | Implemented | Environment allowlisting and output redaction exist. Next: remove unnecessary inherited host identity/path values, cover binary/stream boundary redaction, and verify the sandbox cannot recover stripped secrets elsewhere.                                                                  |
| Timeout and process-tree termination |   Partial   | Process-group termination is automated in the test harness. Next: handle spawn races, escaped/session-reparented descendants, output/resource limits, shutdown recovery, and real Wine/Proton helpers.                                                                                        |
| Approval invalidation                | Implemented | Content and manifest hashes invalidate an in-memory approval. Next: persist approvals safely and bind them to sandbox policy, source provenance, adapter version, interpreter identity, and Lotrex compatibility.                                                                             |
| Safe mode                            |   Partial   | Execution checks safe-mode flags, but third-party adapters/scripts are not integrated into startup and therefore cannot yet be proven disabled before initialization. Add an early main-process gate and recovery UI.                                                                         |
| Save backup before risky actions     |   Partial   | Backup helpers exist in tests. Next: obtain save roots from a trusted game adapter, integrate durable backup metadata/restore verification, and exercise local/cloud-conflict cases in a packaged app.                                                                                        |
| Dry run and declared effects         | Implemented | The API returns a no-write preview, but arbitrary code is not actually simulated. UI must distinguish declarative dry-run evidence from an unknowable raw-script effect set.                                                                                                                  |
| No `sudo` password handling          |   Partial   | Static command-name checks and closed stdin block common cases but are not a privilege boundary and can be bypassed by indirect execution. The sandbox must prohibit privilege acquisition; Lotrex must never collect credentials.                                                            |
