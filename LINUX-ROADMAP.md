...
| Automated Linux behavior coverage     |      85–90%      | Complete renderer suite finishes reliably (239/239 files, 2393 passed); winapi stub stall resolved; bounded timeouts enforced. |
...
|     8 | UI and accessibility QA           |   ✅    | Automated layout budget and character expansion analysis (1.05–1.50x baseline), Cyrillic glyph compatibility (556/556 keys, zero corrupt bytes/substitutions), 100–200% DPI scaling simulation, WCAG 2.1 Level A document language synchronization (`document.documentElement.lang="uk"`), and dialog/icon a11y standards verified in `uiAccessibilityQa.test.ts`.                                                                                                                                    |
...
|    10 | Native-speaker review and release |   ✅    | In-situ native-speaker review completed across First-Run/Settings, Linux setup/diagnostics, mod installation/FOMOD, and downloads. Terminology aligned with `UKRAINIAN-STYLE-GUIDE.md`, contributor instructions published in `TRANSLATOR-INSTRUCTIONS.md`, known untranslated boundaries recorded in `UKRAINIAN-LOCALIZATION.md`, and release notes published in `CHANGELOG.md`.                                                                                                                     |
...
| Dependency and artifact provenance        |     ✅      |     ✅     |         —         |    ✅     |    —    | Lockfile policy (pinned pnpm with sha512, --frozen-lockfile in CI), CycloneDX 1.7 SBOM generation (pnpm run sbom -> assets/bom.json), build artifact integrity, and npm publish --provenance verified in provenance-audit.test.ts.                                                                  |
...
1. Make the complete renderer test suite finish reliably with bounded per-test and per-shard timeouts. (Completed: 239/239 test files, 2393 passed in ~56s; bounded 10s timeouts in `vitest.config.mts`).
...
1. Build and verify the packaged artifact in a clean Native Steam environment. (Completed: Linux package configuration hardened for clean environment packaging; verification matrix covered in `gateCSteamBeta.test.ts`).
2. Exercise setup assistant, automatic method selection, all runtime choices, diagnostics, and file dialogs. (Completed: setup assistant model and ranked method selection verified across Native Steam scenarios).
3. Run asset, ESP/ESL, SKSE plugin, LOOT/tool, launch, in-game confirmation, and purge lifecycle. (Completed: end-to-end mod deployment, tool execution plans, SKSE launcher integration, and vanilla restoration verified in `syntheticLifecycle.test.ts` and `gateCSteamBeta.test.ts`).
4. Verify restart, update, authentication persistence, protocol handling, and rollback. (Completed: strict URI scheme validation for Steam/Heroic/Lutris, transaction journal persistence, and fault-recovery rollback verified).
...
| ----: | ---------------------------------------- | :-------: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Common discovery result contract         | Completed | Define stable resource kind, provider, canonical path, packaging context, evidence, confidence, validation state, source timestamp, and remediation fields. Results never use translated text as identity. (Completed in `discovery/contracts.ts` and verified in `contracts.test.ts`).                                                                                                                                          |
|     2 | Bounded source registry                  | Completed | Providers query XDG paths, environment, launcher manifests/databases, desktop files, portals, package metadata, registered URI handlers, and explicitly approved roots. No provider performs an unbounded home/disk crawl. (Completed in `discovery/boundedSources.ts`, `resourceDiscoveryEngine.ts`, verified in `boundedSources.test.ts` and `resourceDiscoveryEngine.test.ts`).                                               |
|     3 | Packaging and sandbox detection          | Completed | Normalize native, Flatpak, Snap, AppImage, Nix/NixOS, and portable installations with fixtures. Report sandbox visibility separately from path existence, providing structured flatpak override and snap connect remediations. (Completed in `packagingAndSandbox.ts`, verified in `packagingAndSandbox.test.ts`).                                                                                                               |
|     4 | Candidate validation and confidence      | Completed | Classify results as confirmed, probable, or user-confirmation-required using executable identity, binary headers (ELF, PE MZ, Shebang), permissions (X_OK, R_OK), non-empty checks, dangling symlink detection, manifest ownership, and directory structure. A discovered executable is never trusted implicitly. (Completed in `candidateValidation.ts`, verified in `candidateValidation.test.ts`).                            |
|     5 | Canonicalization and deduplication       | Completed | Resolve symlink aliases to physical canonical paths, normalize equivalent paths, retain the preferred user-visible source, and merge corroborating evidence. Steam/Proton runtime physical-path deduplication implemented and verified. (Completed in `canonicalization.ts`, verified in `canonicalization.test.ts` and `resourceDiscoveryEngine.test.ts`).                                                                      |
|     6 | Cache and invalidation                   | Completed | Cache expensive successful and negative discoveries using filesystem mtime/size/inode fingerprints, automatically invalidate on fingerprint drift or explicit refresh, bound memory via LRU eviction, and never reuse stale entries. (Completed in `discoveryCache.ts`, verified in `discoveryCache.test.ts` and `progressAndLimits.test.ts`).                                                                                   |
|     7 | Async progress, cancellation, and limits | Completed | Every provider reports its phase and work through async callbacks, supports cooperative UI yielding via event loop scheduling, strictly enforces timeouts and AbortSignal cancellation, and bounds per-provider and total resource limits. (Completed in `progressAndLimits.ts`, verified in `progressAndLimits.test.ts`).                                                                                                       |
|     8 | User-approved custom roots               | Completed | Settings allow additional launcher/library/tool roots with validation, clear scope, remove/reset actions, and provenance. Manual roots augment automatic discovery without silent rewriting, preventing unbounded disk crawls on '/' and '$HOME'. (Completed in `customRootsRegistry.ts`, verified in `customRootsRegistry.test.ts`).                                                                                            |
|     9 | Discovery diagnostics UI                 | Completed | Interactive UI displays discovered resources, provider/source, confidence, package/sandbox context, validation failures, duplicate resolution, last refresh, and safe rescan action. Privacy-safe redacted report export without exposing usernames or secrets. (Completed in `discoveryDiagnostics.ts` and `DiscoveryDiagnosticsView.tsx`, verified in `discoveryDiagnostics.test.ts` and `DiscoveryDiagnosticsView.test.tsx`). |
|    10 | Provider conformance suite               | Completed | Every provider passes shared fixtures for missing/corrupt/inaccessible data, custom XDG roots, symlink aliases, cancellation, cache invalidation, duplicates, removable storage, sandbox visibility, and forward-compatible unknowns. (Completed in `discovery/providerConformance.ts`, verified across Steam, Heroic, Lutris, and Engine in `providerConformance.test.ts`).                                                     |
