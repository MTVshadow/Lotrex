...
| Interrupted deployment recovery                             |     ✅      |     ✅     |         —         |    ✅     |    —    | Durable per-file plans, checksummed journals, startup inspection, and rollback cover deploy/purge interruption boundaries; packaged process-kill and real-filesystem recovery verification remains.                                                                    |
...
| ------------------------------------- | :--------------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Roadmap feature implementation        |      65–75%      | Many planned services and UI components exist, but discovery and release-gate work still includes prototypes and test-only helpers.                                                 |
| Production-path integration           |      45–55%      | Launcher validation and localization have real consumers; discovery diagnostics, custom roots, portal assessment, and Gate E readiness are not yet wired into normal product flows. |
| Automated Linux behavior coverage     |      70–80%      | Unit and synthetic coverage is broad, but it must not be treated as packaged environment or real-game evidence.                                                                     |
...
| Controlled internal testing stability |      65–75%      | Suitable for internal alpha testing; restricted beta still requires a recorded clean packaged Native Steam lifecycle.                                                               |
| Broad stable-release readiness        |      35–45%      | Recovery foundations exist, but packaged matrices, product integration, release evidence, signatures, and real lifecycle proof remain.                                              |
...
| Complete test-suite reliability   | The full renderer suite previously hung on non-Windows environments due to unhandled thenable proxy stubs.     | Renderer suite completes reliably (239/239 files, 2393 passed); winapi stub thenable trap fixed; bounded 10s test/hook/teardown timeouts enforced in `src/renderer/vitest.config.mts`. |
...
| Hung runtime and launch timeout        |   Partial   |  Partial   |         —         |  Partial  |    —    | An opt-in `VORTEX_PROTON_LAUNCH_TIMEOUT_MS` bounds OS process startup and is disarmed on the child `spawn` event, so it cannot terminate normal gameplay. A future Proton-aware readiness signal is required before prefix initialization or game-window startup can be timed safely. Settings UI and packaged verification remain.                                                                                                    |
...
|    11 | Incremental legacy-key migration     | Partial  | Interface, Application, Updater, Data & Privacy, Meta-server, Download, the Mod Deployment Workaround settings, the Linux Setup assistant, and deployment recovery dialogs/notifications are complete visible-content slices. Download Settings includes controls, guidance, validation, transfer confirmations, progress, cleanup, and recovery errors; obsolete Windows-only and legacy helper copy was removed. Linux Setup assistant and deployment recovery feature dedicated semantic keys for Steam environment detection, Proton runtime selection, staging directory validation, journal rollback/resume confirmations, and recovery details drill-down. English and Ukrainian resources move together. Continue through remaining Settings, navigation, mods/profiles, Nexus, and installers. |
...
|     2 | Language selector in Settings     |   ✅    | The selector discovers `uk`, persists it across sessions, performs atomic restart-free `en -> uk -> en` switching with live translator and UI refresh, handles invalid-locale fallback/rollback, and eliminates obsolete restart copy.                                                                                                                                                                                                                                                                |
...
|     4 | Core navigation and settings      |   ✅    | All currently bundled keys in `common`, `collection`, download, extension, game-mode, mod, Nexus, and profile namespaces are structurally covered. Core navigation (Sidebar, Toolbar, Spine, Header, PageGroup, Menu, WindowControls) and Extension Manager UI migrated to semantic keys and verified in `navigationLocalization.test.ts`.                                                                                                                                                            |
|     5 | Critical Linux workflows          |   ✅    | All 167 source keys in `health_check`, 37 keys for the Linux Setup assistant, and 47 keys for deployment recovery in `mod_management` are translated, covering dependencies, Steam/Proton discovery, prefixes, filesystems, disk space, Flatpak access, remediation, runtime configuration, staging paths, progress indicators, journal inspection, and rollback/resume confirmations. Navigation and installer surfaces (FOMOD dialog, Install From File) translated and covered by automated tests. |
|     6 | Installers and bundled extensions | Partial | Bundled extension namespaces mapped and installer surfaces (`installer_fomod_shared` FOMOD dialog validation/navigation, `mod_management` install archive) translated to semantic keys; fallback defined for untranslated third-party extensions.                                                                                                                                                                                                                                                     |
|     7 | Ukrainian language correctness    |   ✅    | i18next v3 `_0`/`_1`/`_2` Slavic plural forms for counters and relative time exercised with 1/2/5/21/22/25. Unicode NFC/NFD normalization, typographic and modifier apostrophe normalization (`'`, `’`, `ʼ`, `` ` ``), case-insensitive matching, and authentic Ukrainian Cyrillic alphabetical collation verified in `ukrainianCorrectness.test.ts`.                                                                                                                                                 |
|     8 | UI and accessibility QA           | Partial | Automated layout-budget, character-expansion, Cyrillic-glyph, document-language, and dialog/icon checks exist. They are useful regression guards, but DPI simulation is not real rendering and does not establish WCAG conformance. Next: inspect the packaged UI at 100/125/150/200%, complete keyboard-only and screen-reader passes on GNOME and KDE, record clipping/focus/announcement defects, and close them with reproducible evidence.                                                       |
|     9 | Automation and regression gates   |   ✅    | The enhanced `i18n:coverage` command reports every bundled namespace and validates 556/556 keys (100.0% coverage) with zero token/syntax errors, enforces strict mode, and catches obsolete keys and untranslated copies. Comprehensive vitest regression suites cover restart-free switching, table localization, navigation, and language correctness.                                                                                                                                              |
|    10 | Native-speaker review and release | Partial | Style, contributor, localization-boundary, and release-note documents exist, but a complete in-situ packaged-app native-speaker review is not recorded. Next: review First Run/Settings, Linux setup/diagnostics, mod installation/FOMOD, downloads, recovery, and error states in one identified package; log every wording/context issue and retain reviewer/date/commit/package-hash evidence.                                                                                                     |
|    11 | Ongoing maintenance               | Partial | In-app community translation feedback reporter (`TranslationFeedbackDialog.tsx`, `translationFeedback.ts`), automated variable preservation validation, GitHub issue pre-filled link generator, and contributor instructions in `docs/COMMUNITY-TRANSLATIONS.md` implemented and verified in `translationFeedback.test.ts` and `TranslationFeedbackDialog.test.tsx`.                                                                                                                                  |
...
| Workstream                                | Implemented | Integrated | Manually verified | Automated | Blocked | Acceptance criteria                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------- | :---------: | :--------: | :---------------: | :-------: | :-----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Launcher URI and argument injection tests |     ✅      |     ✅     |         —         |    ✅     |    —    | Steam, Heroic, Lutris, Wine, and native plans reject unsafe protocols/identifiers and never concatenate untrusted shell commands.                                                                                                                                                                                                                                                             |
...
| Archive adversarial corpus                |     ✅      |     ✅     |         —         |    ✅     |    —    | Archive metadata is checked before extraction for traversal, path, entry-count, decompressed-size, and ratio limits; the extracted tree is independently checked for lying metadata, escaping symlinks, special devices, and actual size. Packaged malformed/encrypted corpus verification remains.                                                                                           |
| Deployment destination race protection    |     ✅      |     ✅     |         —         |    ✅     |    —    | Destination parent identity and root containment verified before write; descriptor-bound mutations (`/proc/self/fd`) eliminate post-check symlink swapping race windows for backup rename, link creation, restore rename, and unlink operations.                                                                                                                                              |
...
| Dependency and artifact provenance        |   Partial   |  Partial   |         —         |  Partial  |    —    | Pinned pnpm, lockfile integrity, frozen CI installs, CycloneDX SBOM generation, and npm provenance configuration exist. Next: generate provenance for an actual release artifact, publish independently verifiable checksums/signatures, verify the SBOM matches that artifact, and make CI fail when any evidence is absent. String-matching workflow tests do not prove artifact integrity. |

### Deployment TOCTOU remediation plan

Security diff scan `57c362962..b3f4839d2` found four low-severity post-check race windows in
`LinkingDeployment.ts`. Exploitation requires a second local actor with write access to a shared
deployment ancestor, a successful narrow race, and a target accessible to the Lotrex user. They
remain open because pathname validation cannot guarantee that the checked directory is the one used
by the later mutation.

| Phase | Work item                                                                                                                                                                                                                                                                                                                |  Status  | Completion evidence                                                                                                                                                                                                                 |
| ----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :------: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Define the supported attacker and ownership model for game, staging, and deployment roots. Decide whether group/world-writable ancestors are rejected, supported through descriptor-bound operations, or experimental.                                                                                                   | Complete | Documented invariant in `docs/mod-management/DEPLOYMENT-SECURITY-MODEL.md` rejecting world-writable directories without sticky bits, enforcing UID/GID ownership, and requiring descriptor-bound mutations.                         |
|     2 | Introduce one Linux mutation boundary that opens and verifies the destination parent once, then operates relative to that stable directory handle. Prefer `openat2` with beneath/no-symlink constraints plus `linkat`, `symlinkat`, `renameat2`, and `unlinkat`; fail closed when the required primitive is unavailable. | Complete | Implemented `src/renderer/src/util/linux/descriptorMutation.ts` with `/proc/self/fd` capability detection, `O_DIRECTORY` handle pinning, ownership validation, and structured errors.                                               |
|     3 | Migrate backup rename, deployment link creation/replacement, vanilla-backup restore, and cleanup unlink to that boundary while preserving journal order, rollback, cross-device behavior, and activator semantics.                                                                                                       | Complete | Migrated backup rename, link creation, backup restore, and unlinking in `LinkingDeployment.ts` to `openLinuxDestinationHandle` while preserving fault injection checkpoints and journal order.                                      |
|     4 | Add deterministic race regressions that pause after final validation, replace the parent with a symlink, resume, and prove that no path outside `dataPath` is created, renamed, replaced, or removed. Cover hardlink, symlink, move, backup, restore, purge, and `EEXIST` recovery.                                      | Complete | Unit and fault-injection suite in `src/renderer/src/util/linux/descriptorMutation.test.ts` (11/11 passing) verifies symlink substitution immunization, atomic backup/restore, hardlink/symlink creation, and permission boundaries. |
|     5 | Run packaged shared-directory verification with a separate less-privileged account, then update the threat model and support claims.                                                                                                                                                                                     | Deferred | Recorded distro/filesystem/ownership matrix, privacy-safe logs, exact artifact checksum, and recovery result attached to release evidence.                                                                                          |
...
- **Implemented** means production-capable code exists; a fixture, mock, document, or test-only helper is not sufficient.
- **Integrated** means a normal application or CI/release path invokes the implementation without importing test code.
- **Automated evidence** proves only the behavior exercised by that test and never substitutes for packaged/manual evidence.
- **Completed** requires every numbered item in the gate plus a dated evidence record containing the exact commit and artifact checksum.
- If evidence is absent or cannot be reproduced, move the item back to **Partial** or **Pending**.

For each manual or packaged run, store: date, tester, commit, package type and SHA-256, clean/upgrade
starting state, distribution and version, desktop/session, launcher packaging, filesystem and mount,
exact scenario, expected/actual result, redacted logs, discovered defects, and links to retained
evidence. A test filename or generated all-green object is supporting automation, not a run record.

### Gate A — Test foundation

1. Make the complete renderer test suite finish reliably with bounded per-test and per-shard timeouts. (Completed: 263/263 test files, 2578 passed; bounded 10s timeouts in `vitest.config.mts`).
2. Report open handles and the last active test when a shard times out.
3. Run build, typecheck, lint, unit, integration, and synthetic lifecycle checks from a clean checkout. (Latest evidence: typecheck and 239/239 renderer test files pass; lint, format, generated API consistency, build, and synthetic lifecycle checks must pass again after each change set).
...
1. **Partial — build foundation only.** Package configuration and `gateCSteamBeta.test.ts` exist, but no clean packaged Native Steam run is recorded. Next: build one identified artifact, install it under a fresh Linux user, record distro/session/Steam type, commit, package hash, launch logs, and result.
2. **Partial — synthetic coverage only.** Setup models and method ranking are tested. Next: exercise the actual packaged setup assistant, every supported runtime choice, diagnostics, native file dialogs, invalid-path recovery, and restart persistence; attach screenshots or a redacted report.
3. **Pending — real lifecycle required.** Synthetic deployment tests do not prove Nexus, LOOT, Proton, or the game. Next: use one declared reference game and mod set to verify assets, ESP/ESL, SKSE plugin, LOOT/tool execution, game launch, visible in-game confirmation, purge, and vanilla restoration.
4. **Partial — component controls exist.** URI validation, journaling, and rollback have automated coverage. Next: restart the packaged app, upgrade it, confirm authentication/profile/settings persistence, invoke its registered protocol handler, inject a recoverable deployment interruption, and record rollback results.

### Gate D — Flatpak, Snap, desktop, and filesystem matrix

1. **Pending — fixtures only.** Package/session detection is tested, but the native/Flatpak/Snap × Wayland/X11 × KDE/GNOME packaged matrix has not run. Next: define a minimal required matrix, run the same hashed artifact in each cell, and store pass/fail/blocked evidence rather than synthesizing the environment variables in Vitest.
2. **Partial — persistence logic automated.** Encryption and locked-keyring preservation are tested. Next: verify the packaged application against a real unlocked, locked, and unavailable Secret Service, including restart and upgrade, and confirm that failure never overwrites ciphertext.
3. **Partial — remediation helpers are not product-complete.** Portal and Snap command generation is tested, while portal assessment has no normal product consumer. Next: wire assessment into setup/health diagnostics, request only scoped access with user confirmation, re-check access, and verify internal, secondary, and removable libraries in real sandboxes.
4. **Partial — filesystem decisions automated.** ext4/btrfs/EXDEV/NTFS/exFAT branches have synthetic coverage. Next: run deploy/purge/recovery on mounted ext4 and btrfs volumes and confirm safe rejection or guided remediation on representative NTFS and exFAT volumes, including unplug/read-only interruptions.

### Gate E — Stable release

1. **Pending — provenance foundation only.** Lockfile policy, SBOM generation, and npm provenance configuration exist; `gateERelease.test.ts` does not execute a packaged matrix and placeholder hashes are not release evidence. Next: generate the RC once, publish its immutable checksum/SBOM/provenance, and use that exact artifact for every Gate C/D run.
2. **Pending — real blocker review required.** A helper with hard-coded passing fields cannot establish zero blockers. Next: derive each blocker from test results, issue state, packaged reports, and manual sign-off; fail closed on missing/stale evidence and integrate the gate into CI/release promotion.
3. **Partial — documentation draft exists.** Known-limitations and recovery documentation exists, but published artifact checksums/signatures and verified support claims remain. Next: reconcile every claim with the completed matrix, remove unsupported claims, generate signed checksum manifests, and document verification commands.
4. **Pending — promotion prohibited.** Automated rollback tests are useful but do not replace packaged rollback and real-game proof. Next: attach Gate C/D evidence, upgrade/failed-update recovery, real-game lifecycle, unresolved-risk approval, exact commit, and artifact hash to a release record before stable promotion.
...
| ----: | ---------------------------------------- | :---------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Common discovery result contract         | Implemented | Contracts and unit tests define resource identity, evidence, confidence, validation, packaging, and remediation. Next: version the contract, document compatibility rules, and consume it from one production discovery path before declaring integration complete.                                                                                              |
|     2 | Bounded source registry                  | Implemented | Bounded source and engine modules exist with fixtures. Next: audit every provider for bounded I/O, move blocking work off the renderer path, define provider ownership and error isolation, then invoke the registry from application startup or an explicit rescan action.                                                                                      |
|     3 | Packaging and sandbox detection          | Implemented | Native/Flatpak/Snap/AppImage/Nix/portable normalization and remediation models have automated coverage. Next: feed real process/package/portal observations into the production engine and verify them in actual packages rather than environment-variable fixtures alone.                                                                                       |
|     4 | Candidate validation and confidence      | Implemented | Executable, permission, structure, symlink, and confidence helpers exist. Next: make every production provider pass candidates through the validator, bind validation as closely as possible to later use, and add changed-after-discovery handling instead of trusting cached paths indefinitely.                                                               |
|     5 | Canonicalization and deduplication       | Implemented | Canonicalization and evidence merging are tested. Next: integrate them into the product result store, preserve user-facing aliases without confusing identity, and verify bind mounts, removable volumes, inaccessible targets, case-sensitive paths, and paths disappearing mid-scan.                                                                           |
|     6 | Cache and invalidation                   | Implemented | Fingerprint/LRU cache logic exists. Next: connect it to the production engine, persist only safe metadata if persistence is needed, invalidate on launcher/custom-root/settings changes, expose cache age/source in diagnostics, and prove stale results cannot launch or deploy.                                                                                |
|     7 | Async progress, cancellation, and limits |   Partial   | Progress, Promise-based timeout, cancellation, and result-limit helpers exist, but synchronous provider work can block the event loop and defeat timely cancellation. Next: require providers to yield or move blocking parsing/I/O to workers, terminate over-budget work, discard late results, and test elapsed-time bounds with genuinely blocking fixtures. |
|     8 | User-approved custom roots               |   Partial   | `CustomRootsRegistry` and tests exist, but Settings does not expose a complete production flow. Next: mount a settings page, add folder selection/validation/scope/provenance/remove/reset UX, persist roots safely, trigger cache bounded rescans, and reject `/`, home-wide, duplicate or overlapping roots with actionable explanations.                      |
|     9 | Discovery diagnostics UI                 |   Partial   | Diagnostics models and `DiscoveryDiagnosticsView` exist but the view is not mounted in normal navigation. Next: add an accessible Settings/Health Check entry, wire real engine state and cancellation, implement redacted copy/save export, and verify refresh/error/empty/large-result states in the packaged UI.                                              |
|    10 | Provider conformance suite               | Implemented | Steam, Heroic, Lutris, and engine fixtures exercise a shared suite. Next: make conformance mandatory for every registered provider, add real sanitized launcher fixtures and schema-version cases, and keep production provider registrations from bypassing the suite. Passing conformance alone does not mark product integration complete.                    |
...
## Development branch: unified game launcher and modular game support

### Product direction

Lotrex should present one consistent `Lotrex -> game -> profile -> mods/tools` management surface for
native Linux games and Windows games running through Proton or Wine. “One launcher” means one
library, profile, mod, tool, launch, diagnostics, and recovery experience; it does not mean replacing
Steam, Heroic, Lutris, GOG/Epic services, DRM, or their update systems. Lotrex orchestrates installed
games through explicit adapters and hands store-owned operations back to the owning launcher when
required.

The core must remain game-agnostic. Store/launcher discovery, runtime execution, game knowledge, mod
installation, deployment, load order, saves, and external tools are separate capabilities. A game
adapter declares only the capabilities it supports, so adding one game cannot introduce special-case
paths throughout the application.

### Delivery plan

| Phase | Workstream                         | Status  | Implementation direction and completion criteria                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----: | ---------------------------------- | :-----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|     1 | Unified game identity              | Planned | Define an internal identity independent of translated names and install paths: game/edition, platform, store ID, owning launcher, executable, installation, runtime, prefix, and adapter version. Merge duplicate Steam/Heroic/Lutris/manual discoveries without merging different editions. Complete when one installation can move or change launcher path without losing its Lotrex profile.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|     2 | Capability-based game adapter API  | Planned | Replace core game-specific assumptions with versioned capabilities for discovery, mod types, install rules, deployment targets, load order, tools, saves, launch, diagnostics, and migration. Unsupported capabilities must be explicit and visible. Complete when a reference adapter can be installed, upgraded, disabled, and removed without modifying core source or corrupting profiles.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
|     3 | Launcher/provider boundary         | Planned | Give Steam, Heroic, Lutris, Bottles, and manual installations provider adapters that discover and launch but do not own mod semantics. Normalize native executable, Proton, Wine prefix, environment, working directory, and structured arguments into one launch plan. Complete when identical UI actions generate safe, explainable plans for one native and one Windows reference game.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|     4 | Unified library UI                 | Planned | Build one deduplicated library with origin, install state, native/Windows runtime, compatibility status, active mod profile, adapter support level, and launch availability. Include filters and manual correction without silently rewriting launcher data. Complete when users can distinguish duplicate installs and understand why a game cannot yet launch or accept mods.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|     5 | Modular mod pipeline               | Planned | Model discovery/import, archive inspection, installer selection, staging, conflict resolution, deployment, plugin/load-order handling, generated files, purge, and rollback as composable capabilities. Game adapters supply declarative rules or isolated handlers rather than branching the core by game ID. Complete when the same transaction/recovery engine serves two structurally different reference games.                                                                                                                                                                                                                                                                                                                                                                                                                        |
|     6 | Runtime and tool orchestration     | Planned | Use one trusted launch-plan contract for the game and tools such as LOOT, SKSE, xEdit, BodySlide, Nemesis/Pandora, including per-tool runtime/prefix overrides and inherited environment. Never concatenate a shell command. Complete when preview, launch, cancellation, failure diagnostics, and logs work for native, Proton, and Wine processes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
|     7 | User automation and custom scripts | Planned | Provide explicit lifecycle hooks such as before/after install, deploy, purge, tool launch, game launch, backup, and profile switch. Scripts use a versioned manifest with interpreter, arguments, working directory, timeout, required capabilities, supported game/profile scope, and declared inputs/outputs. By default, copy executable content into a per-operation ephemeral workspace, verify its checksum immediately before execution, and remove the workspace after success, failure, cancellation, or next-start recovery. Before enabling, show the exact command and permissions; default to disabled for imported profiles. Complete when scripts can be previewed, individually approved, cancelled, logged, retried, cleaned up, and disabled after failure without bypassing deployment transactions or exposing secrets. |
|     8 | Extension SDK and validation       | Planned | Extend the new-game template into a documented SDK with typed manifests, schema compatibility, fixtures, conformance tests, localization namespaces, permissions, diagnostics, and sample native/Proton adapters. Complete when a contributor can add a game without searching core code for paths or user-visible strings.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|     9 | Game support tiers and catalog     | Planned | Track `experimental`, `community-tested`, and `supported` per game/edition/store/runtime—not merely per title. Record maintainer, adapter version, tested artifact, distro/runtime/deployment matrix, known limitations, and last verification date. Promotion requires reproducible evidence; stale evidence automatically downgrades the claim.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
|    10 | Reference-game pilot               | Planned | Select a deliberately small pilot: one native Linux game and one Windows/Proton game with materially different mod layouts. Verify clean discovery, profile creation, mod install, conflicts, deployment, tools and scripts where applicable, launch, visible in-game result, purge, recovery, restart, and upgrade using identified packaged artifacts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|    11 | Controlled library expansion       | Planned | Add games in small cohorts only after the adapter SDK and pilot are stable. Prioritize games with maintainers, legal redistributable fixtures, well-understood mod formats, and repeatable smoke scenarios. A large catalog is not a success metric if its entries cannot pass their declared support tier.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

### Architecture boundaries

- The **launcher provider** knows where a game came from and how its owner expects it to start.
- The **runtime provider** turns a native, Proton, Wine, or container choice into a structured process plan.
- The **game adapter** knows game-specific directories, mod types, plugins, saves, tools, and validation.
- The **mod pipeline** owns archive safety, staging, conflicts, transactional deployment, purge, and recovery.
- The **profile layer** binds a game installation, runtime, mods, tools, and saves without binding identity to an absolute path.
- The **automation layer** exposes audited lifecycle events and runs user-approved scripts without giving adapters an unrestricted execution shortcut.
- The **core UI** consumes these contracts and must not contain per-game or per-launcher path heuristics.

### Scope and safety rules

- Stabilize one native and one Proton reference game before expanding the catalog.
...
- Store clients retain ownership of purchase, download, DRM, game updates, cloud saves, and account sessions unless a documented API explicitly permits integration.
- Adapter code receives least privilege, declares required roots/capabilities, and cannot bypass path containment, archive limits, redaction, launch validation, or the deployment journal.
- Custom scripts are treated as executable code, never as trusted mod metadata. Importing a mod, collection, profile, or adapter must not execute or silently approve a script.
- A script runs from a private per-operation workspace, never directly from Downloads, a mod archive, staging, or a launcher-owned directory. Its checksum is verified after copying and immediately before process creation.
- Temporary workspaces use unique operation IDs, restrictive permissions, size/time limits, deterministic cleanup, and startup reconciliation; cleanup must never follow untrusted symlinks or remove an unverified directory tree.
- Prefer structured actions for copying, patching, launching, and environment changes. Raw shell mode is advanced, visibly marked, uses argument arrays instead of concatenation where possible, has bounded execution, and requires separate approval for network, external paths, or privilege escalation.
- Script output and environment are redacted before logging; credentials and unrelated host environment variables are not inherited by default. Lotrex never asks for or stores a `sudo` password for a script.
- Script failures cannot commit a deployment transaction. Hooks that modify managed files must declare outputs and participate in backup, rollback, generated-file ownership, and recovery.
- Prefer declarative adapter data; isolate unavoidable executable code behind a versioned, testable boundary with failure containment.
- Preserve unknown adapter data during upgrades and provide export, disable, rollback, and safe-mode recovery.
- Do not add a new game to `supported` until its exact edition/store/runtime combination has a retained packaged lifecycle record.

### Custom script contract

The first version should support executable files and explicit interpreters such as Bash, Python, or
Node only when they are already available and selected by the user. Missing interpreters produce a
diagnostic rather than an automatic system-wide installation. A portable declarative action format
should cover common file operations so most extensions do not need shell access.

User scripts and helper executables are **ephemeral by default**. Lotrex copies the approved,
checksum-identified input into a newly created operation directory with restrictive permissions,
executes only that copy, records declared outputs, terminates its process tree, and removes the
directory on every exit path. Startup recovery removes abandoned workspaces only after verifying
that they are Lotrex-owned operation directories. Persistent installed tools are a separate,
explicitly approved feature with ownership, version, update, uninstall, and integrity tracking.

Ephemeral execution is cleanup and integrity hygiene, not a security boundary: a temporary program
can still alter every host file it can access. Actual isolation therefore requires restricted
filesystem, process, and network access plus transactional handling of all declared managed-file
writes. Where the platform cannot enforce the requested isolation, Lotrex must display that
limitation and fail closed for scripts requiring stronger guarantees.

Every script manifest must declare:

- stable ID, display name, author/source, version, checksum, and compatible Lotrex/adapter versions;
- lifecycle event and ordering, game/profile scope, interpreter/executable, argument array, working
  directory, timeout, and cancellation behavior;
- allowed read/write roots, network requirement, environment-variable allowlist, expected outputs,
  temporary workspace budget, and whether the hook is transactional;
- exit-code policy, retry/idempotency behavior, privacy-safe log policy, and rollback/cleanup action.

Settings must provide a per-game/profile script list with enable/disable, reorder, inspect, approve,
run manually, view redacted logs, and reset permissions. A safe mode starts Lotrex with all custom
scripts disabled. Shared scripts may be imported, but their checksum or permission changes revoke
previous approval and require a new exact preview.

### Mandatory custom-script security gate

Custom scripting remains experimental and disabled by default until every mandatory control below
is implemented, integrated, and tested. A mod, collection, game adapter, imported profile, migration,
or update may propose a script but can never enable, approve, or execute it.

| Control                              | Status  | Required implementation and acceptance evidence                                                                                                                                                                                                                                                  |
| ------------------------------------ | :-----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| No automatic execution               | Planned | Every new/imported/updated script starts disabled. Only a direct user action in Lotrex may approve and run it. Regression tests cover mod, collection, adapter, profile, update, and dependency-install paths and prove none can trigger execution.                                              |
| Minimum filesystem permissions       | Planned | Grant only separately declared read/write roots for the selected game installation, its staging area, and active profile. Read and write permissions are distinct; undeclared access fails and produces a redacted diagnostic.                                                                   |
| Forbidden host areas                 | Planned | Reject `/`, the whole home directory, system locations, SSH/GPG configuration, keyring/credential storage, browser profiles, unrelated launcher data, and other games/profiles/prefixes. Resolve symlinks, bind mounts, parent substitutions, and canonical paths before enforcing the boundary. |
| Exact pre-execution preview          | Planned | Show script identity/source/checksum, executable or interpreter, argument array, working directory, lifecycle event, environment allowlist, readable/writable roots, network access, timeout, expected outputs, transaction behavior, and cleanup plan before approval.                          |
| Transactional managed-file writes    | Planned | Produce an authoritative change plan before mutation; journal, back up, attribute, and roll back every managed output. A failed/cancelled script cannot commit deployment state, and unexpected outputs fail safely.                                                                             |
| Isolated Proton/Wine prefixes        | Planned | Bind a script to one explicitly selected game/profile prefix. Other prefixes are inaccessible; shared-prefix use requires a separate high-risk confirmation and backup. Tests cover path aliases and attempts to escape through Wine drive mappings.                                             |
| Sanitized process environment        | Planned | Start from a minimal allowlist rather than inheriting the full Lotrex environment. Do not expose tokens, cookies, keyring handles, unrelated paths, debug secrets, or launcher credentials; redact output before persistence or display.                                                         |
| Timeout and process-tree termination | Planned | Enforce wall-clock, idle, output, memory, process-count, and workspace-size limits where supported. Cancellation and timeout terminate descendants, Wine/Proton helpers, and watchers without leaving an operation marked successful.                                                            |
| Approval invalidation                | Planned | Bind approval to script checksum, manifest, interpreter, arguments, permissions, lifecycle event, source, and adapter version. Any change revokes approval and shows a new exact diff/preview.                                                                                                   |
| Safe mode                            | Planned | Provide a documented startup path that disables all third-party adapters, scripts, and pending automation before they initialize. Users can inspect, export logs, revoke approvals, and recover profiles without executing extension code.                                                       |
| Save backup before risky actions     | Planned | Detect game-specific save roots through the adapter, create an atomic identified backup before a script may write to them, verify restore, apply retention rules, and never silently include cloud credentials or unrelated saves.                                                               |
| Dry run and declared effects         | Planned | Prefer declarative actions and produce a no-write preview where operations are predictable. Clearly label when arbitrary executable behavior makes a complete dry run impossible; lack of predictability requires stronger isolation or refusal.                                                 |
| No `sudo` password handling          | Planned | Lotrex and scripts never request, receive, pipe, log, or store a `sudo` password. Privileged changes are outside the script API; documented system administration remains a separate user-controlled action.                                                                                     |

The gate passes only when the controls are exercised against benign fixtures and adversarial cases:
path traversal, symlink/bind-mount escape, checksum replacement, environment-secret access, forked and
detached processes, Wine `Z:` access, cross-prefix writes, undeclared outputs, cancellation, timeout,
crash recovery, and malicious imported manifests. Packaged verification must use an unprivileged test
account and record the exact artifact checksum and sandbox/filesystem configuration.
