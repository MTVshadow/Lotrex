...
|     9 | Community locale scaffolding         |    —    | Add `pnpm run i18n:create -- <locale>` to validate the locale code, copy the canonical English template, create metadata, preserve key order/structure, refuse destructive overwrite, and print the exact validation/preview workflow.                                                                                                                                                                                                                                                                                                             |
|    10 | Validator and local preview          | Partial | Extend the current coverage command to detect missing/obsolete keys, empty values, invalid JSON, protected-token/tag drift, plural forms, accidental source-language copy, namespace collisions, and incompatible metadata. Add a locale-only development preview/reload path with actionable errors.                                                                                                                                                                                                                                              |
|    11 | Incremental legacy-key migration     | Partial | Interface, Application, Updater, Data & Privacy, Meta-server, Download, and the Mod Deployment Workaround settings are complete visible-content slices. Download Settings includes controls, guidance, validation, transfer confirmations, progress, cleanup, and recovery errors; obsolete Windows-only and legacy helper copy was removed. English and Ukrainian resources move together. Continue through remaining Settings, Linux setup, deployment/recovery, navigation, mods/profiles, Nexus, and installers.                               |
...
|    14 | Translator documentation             |    —    | Provide `TRANSLATING.md` with a no-code copy/translate/validate/preview/PR path, JSON examples, glossary guidance, variables/plurals/markup rules, screenshots, review expectations, attribution, license terms, and a correction workflow suitable for first-time contributors.                                                                                                                                                                                                                                                                   |
...
|     5 | Critical Linux workflows          | Partial | All 167 source keys in `health_check` are translated, including dependencies, installation/version selection, feedback, Premium guidance, Steam/Proton discovery, prefixes, filesystems, disk space, permissions, Flatpak access, command preview, reports, and remediation. Setup, deployment/purge, recovery and runtime-selection surfaces outside this namespace plus packaged user review remain.           |
...
|     9 | Automation and regression gates   | Partial | The persistent coverage command reports every bundled namespace and fails on empty translations or mismatched interpolation/component tokens; the current 361/361 run passes. Obsolete-key, accidental-English, complete plural/fallback, legacy source-key inventory, and CI gates remain.                                                                                                                      |
