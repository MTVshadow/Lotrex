...
/**
 * Educational Comment:
 * Determines whether an error was caused by a disconnected, unmounted, or stale storage volume
 * (e.g. unannounced physical removal of a USB flash drive or external SSD).
 * Standard POSIX indicators include:
 * - ENODEV: No such device
 * - ENXIO: No such device or address
 * - ESTALE: Stale file handle (common in NFS / FUSE / removable mounts)
 * - EIO: Input/output error generated by sudden block device disappearance
 * - EREMOTEIO: Remote I/O error reported by removable/FUSE/network-backed storage
 * - EDEPLOYMENTVOLUMEUNAVAILABLE: Vortex domain code for missing deployment roots
 */
export function isVolumeUnavailableError(err: unknown): boolean {
  const code =
    getErrorCode(err) ??
    (typeof err === "object" &&
    err !== null &&
  ...
  try {
    ...
    if (isVolumeUnavailableError(err)) {
      const unavailable = new Error(
        `Deployment volume became unavailable during file mutation: ${getErrorMessageOrDefault(err)}`,
        { cause: err },
      );
      unavailable["code"] = "EDEPLOYMENTVOLUMEUNAVAILABLE";
      throw unavailable;
...
async function reconcileFileOperation(
  ...
  try {
    ...
    if (isVolumeUnavailableError(err)) {
      const unavailable = new Error(
        `Deployment volume became unavailable during file reconciliation at ${operation.targetPath}: ${getErrorMessageOrDefault(err)}`,
        { cause: err },
      );
      unavailable["code"] = "EDEPLOYMENTVOLUMEUNAVAILABLE";
      unavailable["path"] = operation.targetPath;
      throw unavailable;
    }
    return {
      operation,
  ...
  try {
    if (operation.action === "deploy") {
      if (reconciliation.state === "applied") {
        ...
        operation.recoveryState !== "rolling-back"
      ) {
    ...
    if (isVolumeUnavailableError(err)) {
      const unavailable = new Error(
        `Deployment volume became unavailable during file rollback at ${operation.targetPath}: ${getErrorMessageOrDefault(err)}`,
        { cause: err },
      );
      unavailable["code"] = "EDEPLOYMENTVOLUMEUNAVAILABLE";
      unavailable["path"] = operation.targetPath;
